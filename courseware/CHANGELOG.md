# Changelog

## v9.0 — 20 August 2026

- Rebuilt the 2-day WSQ deck in the current visual component system.
- Added a topic-specific generated cover hero and editable native PowerPoint charts.
- Moved detailed procedures out of the deck and into the Learner Guide and lab guides.
- Created eight progressive, self-contained labs with native Inventor sample files.
- Rebuilt the Learner Guide and Lesson Plan from the same course source and current slide map.
- Revised the 8-question WA and 6-task PP while preserving K1–K8, A1–A11 and instrument type.
- Prepared Drive/TMS publication metadata and the public-only GitHub release boundary.

## v8.0 — 16 July 2026

- Legacy trainer deck and delivery package retained as the prior release authority.

